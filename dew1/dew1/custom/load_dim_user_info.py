...
def transform_custom(wh_table: dict, *args, **kwargs):
    ...
    schema = Schema(
        ...
        NestedField(field_id=2, name="uid", field_type=StringType(), required=False),
        NestedField(field_id=3, name="password", field_type=StringType(), required=False),
        NestedField(field_id=4, name="first_name", field_type=StringType(), required=False),
        NestedField(field_id=5, name="last_name", field_type=StringType(), required=False),
        NestedField(field_id=6, name="username", field_type=StringType(), required=False),
        NestedField(field_id=7, name="email", field_type=StringType(), required=False),
        NestedField(field_id=8, name="avatar", field_type=StringType(), required=False),
        NestedField(field_id=9, name="gender", field_type=StringType(), required=False),
        NestedField(field_id=10, name="phone_number", field_type=StringType(), required=False),
        NestedField(field_id=11, name="social_insurance_number", field_type=StringType(), required=False),
        NestedField(field_id=12, name="date_of_birth", field_type=TimestampType(), required=False),
        NestedField(field_id=13, name="credit_card_cc_number", field_type=StringType(), required=False),
        NestedField(field_id=14, name="row_id", field_type=StringType(), required=False),
        NestedField(field_id=15, name="datetime_day", field_type=TimestampType(), required=False),
        # Additional fields for SCD Type 2
        NestedField(field_id=16, name="effective_date", field_type=TimestampType(), required=False),
        NestedField(field_id=17, name="end_date", field_type=TimestampType(), required=False),
        NestedField(field_id=18, name="is_current", field_type=BooleanType(), required=False)
    )
    ...
    if not hive_catalog.table_exists("lakehouse_w.dim_user_info"):
        table = hive_catalog.create_table(
            "lakehouse_w.dim_user_info",
            schema=schema,
        ...
        table = hive_catalog.load_table("lakehouse_w.dim_user_info")
    ...
    for _, record in arrow_df.to_pandas().iterrows():
        ...
        if not len(existing_df) == 0:
            ...
            arrow_df = arrow_df.set_column(
                11,
                'date_of_birth',
                arrow_df['date_of_birth'].cast(pa.timestamp('us'))
            )
            arrow_df = arrow_df.set_column(
                14,
                'datetime_day',
        ...
        new_record_arrow = new_record_arrow.set_column(
            11,
            'date_of_birth',
            new_record_arrow['date_of_birth'].cast(pa.timestamp('us'))
        )
        new_record_arrow = new_record_arrow.set_column(
            14,
            'datetime_day',
